...
const defaultHeartImages = Array.from({ length: 3 }, (_, i) => `images/img${i + 1}.gif`);
...
// Point density when there are 10 or more images (lowest)
const minDensity = 2000;
// Maximum number of images we care about for scaling
...
console.log(`Number of images: ${numGroups}, Points per image: ${pointsPerGroup}`);
...
for (let i = 0; i < galaxyParameters.count; i++) {
  ...
  const randomY = (Math.random() - 0.5) * galaxyParameters.randomness * radius * 1.2; // changed from 0.5 to 1.5
...
const galaxyMaterial = new THREE.ShaderMaterial({
  ...
  vertexShader: `
        ...
        void main() {
            // Get base color from geometry (same as vertexColors: true)
            vColor = color;
            ...
            if (uRippleTime > 0.0) {
                ...
                if (strength > 0.0) {
                    vColor += vec3(strength * 2.0); // Make color brighter when ripple passes
                }
            }

            vec4 viewPosition = viewMatrix * modelPosition;
            gl_Position = projectionMatrix * viewPosition;
            // This line makes points smaller when farther away, simulating PointsMaterial behavior
...
// ---- CREATE HEART-SHAPED POINT GROUPS ----
for (let group = 0; group < numGroups; group++) {
  ...
  // Geometry for near-camera state
  ...
  // Calculate group center and translate to origin
  ...
  img.onload = () => {
    ...
    pointsObject.position.set(cx, cy, cz); // Set back to original position in scene

    // Store states for switching later
...
// Function to create planet texture
function createPlanetTexture(size = 512) {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    spotGradient.addColorStop(0, color + 'cc'); // 'cc' is alpha
...
// ---- CREATE ROTATING TEXT RINGS AROUND PLANET ----
const ringTexts = [
  'Miwoup',
  "putigiaa",
  "Spesial Untukmu",
...
function createTextRings() {
  ...
  for (let i = 0; i < numRings; i++) {
    const text = ringTexts[i % ringTexts.length] + '   '; // Add spaces
    const ringRadius = baseRingRadius + i * ringSpacing;

    // ---- Font size analysis logic (unchanged) ----
    ...
    // Glow effect for text border
    ctx.shadowColor = '#e0b3ff';
    ctx.shadowBlur = 18;
    ctx.lineWidth = 7;
    ctx.strokeStyle = '#fff';
    ctx.strokeText(fullText, 0, textureHeight * 0.82); // lower line
    ...
    ringGroup.userData = {
      ...
      speed: 0.002 + 0.00025, // Rotation speed
      tiltSpeed: 0, rollSpeed: 0, pitchSpeed: 0, // Sway speed
      tiltAmplitude: Math.PI / 3, rollAmplitude: Math.PI / 6, pitchAmplitude: Math.PI / 8, // Sway amplitude
      tiltPhase: Math.PI * 2, rollPhase: Math.PI * 2, pitchPhase: Math.PI * 2, // Sway phase
      isTextRing: true
    };
...
// =======================================================================
// ---- ADD HINT ICON TO SUGGEST CLICKING THE PLANET ----
// =======================================================================

let hintIcon;
let hintText;
/**
 * Create a 3D cursor icon to suggest user interaction.
 * FINAL VERSION: Solid white cursor placed further from the central planet.
 */
function createHintIcon() {
  ...
  // --- 1. CREATE CURSOR SHAPE (Unchanged) ---
  ...
  // Background layer (was black border, now white background)
  const backgroundGeometry = new THREE.ShapeGeometry(cursorShape);
  const backgroundMaterial = new THREE.MeshBasicMaterial({
    color: 0xffffff, // CHANGED: Border to white
    side: THREE.DoubleSide
  });
  const backgroundMesh = new THREE.Mesh(backgroundGeometry, backgroundMaterial);

  // Inner white layer (not necessary now but kept for thickness)
  const foregroundGeometry = new THREE.ShapeGeometry(cursorShape);
  const foregroundMaterial = new THREE.MeshBasicMaterial({
    color: 0xffffff, // Keep white
    side: THREE.DoubleSide
  });
  ...
  // --- 3. CREATE SURROUNDING RING (Unchanged) ---
  ...
  // CHANGED: Place icon further away
  hintIcon.position.set(1.5, 1.5, 15); // Increase Z from 12 to 20
...
function animateHintIcon(time) {
  ...
  if (!introStarted) {
    ...
    // Handle hint text (add new effect)
    if (hintText) {
...
function animate() {
  ...
  // Switch material for heart point groups
  scene.traverse(obj => {
...
createShootingStar();
createHintIcon(); // Call icon creation
createHintText();
...
// Limit number of stars shown initially
...
function onCanvasClick(event) {
  ...
  if (intersects.length > 0) {
    ...
    playGalaxyAudio(); // Preload music on script load
...
// =======================================================================
// ---- CHECK SCREEN ORIENTATION TO SHOW WARNING ----
// =======================================================================

function checkOrientation() {
  // Check if height > width (portrait mode on mobile)
...
// Listen for events to recheck orientation
window.addEventListener('DOMContentLoaded', checkOrientation);
window.addEventListener('resize', checkOrientation);
window.addEventListener('orientationchange', () => {
  // Add delay for browser to update size
